...
function updateItemsList() {
  ...
  ajaxSecureLoadVariables("pablotests", { itemList: null }, function() {
    ...
    for (var i = 0; i < itemList.length; i++) {
      ...
      itemText.innerHTML = itemList[i].item.concat(
        " level ",
        itemList[i].Level
      );
      ...
      button.style.backgroundImage = "url(../../secondwind/pabloNEW/images/".concat(
        itemList[i].item,
        ".svg)"
      );
...
function retrieveCombinationTimes() {
  ...
  ajaxSecureLoadVariables(
    "ghost",
    { ajaxGetFinishedCombinations: null },
    function() {
      updateItemsList();
  ...
  ajaxSecureLoadVariables(
    "ghost",
    { ajaxGetOngoingCombinations: null },
    function() {
      ...
      for (var i = 0; i < ajaxGetOngoingCombinations.length; i++) {
        //creates ongoing combination elements
        var combinationText = document.createTextNode(
          ajaxGetOngoingCombinations[i].item1 +
            " is being combined with " +
            ajaxGetOngoingCombinations[i].item2 +
            "\n"
        );
        var combinationBar = document.createElement("progress_bar");
      ...
      document.getElementById("updateCombinations").innerHTML =
        "Update Combinations";
    }
  );
}
...
function addToCombine(id) {
  ...
  //displays the two elements that will be combined
  document.getElementById("testItems").innerHTML = el1
    .concat(" will be combined with ")
    .concat(el2);
}
...
function combineItems() {
  ...
  ajaxSecureCall(
    "ajaxCombineItems",
    { id: uniqueId, el1: el1, level1: level1, el2: el2, level2: level2 },
    function() {
...
function notEnoughItems() {
  document.getElementById("errorItems").innerHTML =
    "Sorry babe you don't have enough of one of the items (this message will be improved to say which item in the future";
}
